...
public class Entity: ITargeter, ITargetable, IUser  
{
    ...
    ***************************************/
    ...
    {
        if (this._targeter == null) return false;
        return this._targeter.CanTarget(candidate);
    }

    public int GetMaxTargets(object source, IEnumerable<ITargetable> candidates)
    {
        if (this._targeter == null) return 0;
        return this._targeter.GetMaxTargets(source, candidates);
    }

    public IEnumerable<ITargetable> GetEligibleTargets(object source, IEnumerable<ITargetable> candidates)
    {
        if (this._targeter == null) return new List<ITargetable>();
        return this._targeter.GetEligibleTargets(source, candidates);
    }

    public void ClearTargets()
    {
        if (this._targeter == null) return;
    ...
    *********************/
    ...
    public UsableResult Use(IUsable usable, Entity user, Entity target)
    {
        ...
        return this._user.Use(usable, user, target);
    }
}